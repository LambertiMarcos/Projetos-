...
def make_video_mpg_plot(figure, gs = None, pddata = None, axes = None):
    if not axes:
        axes = figure.add_subplot(gs[0, 0])
...
def make_video_views_plot(figure, gs = None, pddata = None, axes = None):
    if not axes:
        axes = figure.add_subplot(gs[0, 0])
    ...
    make_ids = geradados.get_title_ids(pddata)
    axes.set_title("Videos Views Ranges", fontsize=20)
    ...
    axes.set_xticklabels([" "] + geradados.get_title_labels(pddata))
    axes.set_xlabel("Title", fontsize=16)
